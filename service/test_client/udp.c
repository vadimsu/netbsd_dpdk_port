...
int main(int argc,char **argv)
{
    ...
    if(service_app_init(argc,argv,"udp") != 0) {
...
printf("bind to %s %d\n",my_ip_addr,port_to_bind + i);
    ...
    p_timeout = &tm_out;
    while(1) {
        ready_socket = service_select(selector,&readfdset,&writefdset,&excfdset,p_timeout);
        if(ready_socket == -1) {
            continue;
        }

        for (ready_socket = 0; ready_socket < readfdset.returned_idx; ready_socket++) {
	    if (!service_fdisset(service_fd_idx2sock(&readfdset,ready_socket),&readfdset))
		continue;
	    p_timeout = NULL;
	    len = 1448;
	    ...
	    if (!usebulk) {
	            for(i = 0;i < tx_space;i++) {
        	        txbuff = service_get_buffer(DATAGRAM_SIZE,service_fd_idx2sock(&writefdset,ready_socket),&pdesc);
                	if(txbuff) { 
#if USE_CONNECTED
	                    if(service_send(service_fd_idx2sock(&writefdset,ready_socket),pdesc,0,DATAGRAM_SIZE)) {
			    ...
			    in_addr->sin_addr.s_addr = inet_addr(ip_addr_2_connect);
			    in_addr->sin_port = port_to_connect;
                	    if(service_sendto(service_fd_idx2sock(&writefdset,ready_socket),pdesc,0,DATAGRAM_SIZE,&addr,addrlen)) {
                        	service_release_tx_buffer(pdesc);
	                    }
			    else
				transmitted_packets++;
		...
		struct sockaddr addresses[tx_space];
				...
				in_addr->sin_family = AF_INET;
                           ...
                           if(service_sendto_bulk(service_fd_idx2sock(&writefdset,sock),
